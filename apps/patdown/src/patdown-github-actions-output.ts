import type { PatdownGitHubAnnotationLevel } from '@patdown/rules'
import { defaultPatdownGitHubAnnotationLevel } from '@patdown/rules'
 ...
 */
export function makePatdownGitHubActionsOutputLive(
	configuredAnnotationLevel: PatdownGitHubAnnotationLevel = defaultPatdownGitHubAnnotationLevel,
): Layer.Layer<PatdownOutput, never, FileSystem.FileSystem> {
	return Layer.effect(
		PatdownOutput,
		Effect.gen(function* () {
			...
			const publish = (
				...
				Effect.gen(function* () {
					for (const line of formatPatdownGitHubActionsAnnotations(
						results,
						configuredAnnotationLevel,
					)) {
...
/** Default GitHub Actions output: FAIL annotations use error unless configured elsewhere. */
export const PatdownGitHubActionsOutputLive = makePatdownGitHubActionsOutputLive()
