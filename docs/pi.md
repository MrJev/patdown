...
Read-only tools are ignored. Bash is ignored, including `cat > file` / `tee` / `sed -i`. Missing rules disable judging instead of blocking every write. On session start, Pi appends that skip to the conversation and shows it as a notice, so a quiet session is not a pass. Frontmatter `include:` in `AGENTS.PATDOWN.md` is followed (pack directories or individual rule files), so packs and project rules compose the same way as the CLI. A plain `loadPatdownRules` adapter works here too.
